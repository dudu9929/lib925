module Bookable
  ...
  def start_date_cannot_be_in_the_past
    if start_time && start_time < DateTime.now + (1.minutes)
      errors.add(:start_time, 'must be earlier present time')
    else
      if start_time && start_time > DateTime.now + (7.days)
        errors.add(:start_time, 'must be at most 7 days from present time')
